...
function customCompletions(profile) {
  return isRecord(profile)
    && typeof profile.baseURL === 'string'
    && profile.baseURL.length > 0
    && (profile.api === undefined || profile.api === 'openai-completions')
}
...
function listPayload(settings, state) {
  ...
  const providers = Object.entries(effective).flatMap(([id, profile]) => {
    if (!customCompletions(profile)) return []
...
async function runProbe(ctx, state, body) {
  ...
  if (!customCompletions(profile) || !modelRows(profile).some(row => row.id === model)) {
  ...
  try {
    ...
    cleanup(state)
    state.reports.set(report.reportId, {
      provider,
      model,
      proposal: structuredClone(report.proposal),
...
async function runApply(ctx, state, body) {
  ...
  await ctx.settings.mutate(NS, [{ op: 'set', path: ['providers', cached.provider, 'models'], value: nextModels }], descriptor.revision)
