window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PassportCard(props) {
      ...
      var dataState = React.useState({ providers: [], requestPlan: {} }), data = dataState[0], setData = dataState[1]
      ...
      function runProbe() {
        ...
        post(API.probe, { provider: provider, model: model, confirmed: true }).then(function done(next) {
          ...
          setBusy('')
          if (!next || next.ok !== true) { setMessage((next && next.error) || 'probe-failed'); return }
      ...
      function applyProfile() {
        ...
        post(API.apply, { reportId: result && result.reportId }).then(function done(next) {
          setBusy('')
          if (!next || next.ok !== true) { setMessage((next && next.error) || 'apply-failed'); return }
